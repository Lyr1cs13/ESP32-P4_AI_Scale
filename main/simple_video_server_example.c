...
#define EXAMPLE_HX711_DATA_GPIO            GPIO_NUM_22
#define EXAMPLE_HX711_SCK_GPIO             GPIO_NUM_23
/* Same calibrated scale factor used by the standalone HX711 example. */
#define EXAMPLE_HX711_SCALE                380.0f
#define EXAMPLE_HX711_AVG_SAMPLES          10
#define EXAMPLE_WEIGHT_POLL_INTERVAL_MS    100
#define EXAMPLE_WEIGHT_CHANGE_THRESHOLD_G  15.0f
#define EXAMPLE_WEIGHT_STABLE_DELTA_G      5.0f
#define EXAMPLE_WEIGHT_STABLE_COUNT        2
#define EXAMPLE_TRIGGER_COOLDOWN_MS        2500
#define EXAMPLE_HX711_POWER_UP_DELAY_MS    1000
#define EXAMPLE_AI_CAPTURE_TIMEOUT_MS      1000
...
extern void ai_inference_task(void *arg);
...
typedef struct latest_frame_buffer {
    uint8_t *buf;
    uint32_t capacity;
    uint32_t size;
    uint32_t width;
...
static latest_frame_buffer_t s_latest_frame = {0};
static TaskHandle_t s_ai_task_handle = NULL;
static web_cam_t *s_web_cam = NULL;
static web_cam_video_t *s_ai_input_video = NULL;
static const uint32_t AI_TASK_WDT_TIMEOUT_MS = 20000;
static bool s_weight_trigger_enabled = false;
...
static web_cam_video_t *get_ai_input_video(web_cam_t *web_cam)
{
    if (!web_cam) {
        return NULL;
    }
    ...
    for (int i = 0; i < web_cam->video_count; i++) {
        ...
        if (web_cam->video[i].pixel_format == V4L2_PIX_FMT_RGB565) {
...
static esp_err_t latest_frame_init(web_cam_video_t *video)
{
    ...
    ESP_RETURN_ON_FALSE(s_latest_frame.buf, ESP_ERR_NO_MEM, TAG, "failed to alloc latest frame buffer");

    s_latest_frame.lock = xSemaphoreCreateMutex();
    ESP_RETURN_ON_FALSE(s_latest_frame.lock, ESP_ERR_NO_MEM, TAG, "failed to create latest frame lock");

    s_latest_frame.width = video->width;
    s_latest_frame.height = video->height;
    s_latest_frame.pixel_format = video->pixel_format;
    s_latest_frame.size = 0;
    s_latest_frame.valid = false;

    ESP_LOGI(TAG, "latest frame buffer init ok, capacity=%" PRIu32 " format=" V4L2_FMT_STR,
             s_latest_frame.capacity, V4L2_FMT_STR_ARG(video->pixel_format));
    return ESP_OK;
}

static esp_err_t latest_frame_update(web_cam_video_t *video, struct v4l2_buffer *buf)
{
    ESP_RETURN_ON_FALSE(s_latest_frame.buf, ESP_ERR_INVALID_STATE, TAG, "latest frame buffer not initialized");
    ...
    ESP_RETURN_ON_FALSE(xSemaphoreTake(s_latest_frame.lock, portMAX_DELAY) == pdPASS,
                        ESP_FAIL, TAG, "failed to lock latest frame buffer");
...
static esp_err_t capture_frame_to_latest_buffer(web_cam_video_t *video)
{
    struct v4l2_buffer buf;
    struct timeval timeout = {
        .tv_sec = EXAMPLE_AI_CAPTURE_TIMEOUT_MS / 1000,
        .tv_usec = (EXAMPLE_AI_CAPTURE_TIMEOUT_MS % 1000) * 1000,
    };
    fd_set read_fds;
    esp_err_t ret;
    int ready;

    ESP_RETURN_ON_FALSE(video, ESP_ERR_INVALID_STATE, TAG, "AI input video is not ready");

    FD_ZERO(&read_fds);
    FD_SET(video->fd, &read_fds);
    ready = select(video->fd + 1, &read_fds, NULL, NULL, &timeout);
    ESP_RETURN_ON_FALSE(ready > 0, ESP_ERR_TIMEOUT, TAG, "timeout waiting for AI capture frame");

    memset(&buf, 0, sizeof(buf));
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;

    ESP_RETURN_ON_ERROR(ioctl(video->fd, VIDIOC_DQBUF, &buf), TAG, "failed to receive video frame for AI");
    if (!(buf.flags & V4L2_BUF_FLAG_DONE)) {
        ioctl(video->fd, VIDIOC_QBUF, &buf);
        return ESP_ERR_INVALID_RESPONSE;
    }

    ret = latest_frame_update(video, &buf);
    ioctl(video->fd, VIDIOC_QBUF, &buf);
    ESP_RETURN_ON_ERROR(ret, TAG, "failed to update latest frame");

    ESP_LOGI(TAG, "captured one frame for AI, size=%" PRIu32, buf.bytesused);
    return ESP_OK;
}

static esp_err_t trigger_capture_from_weight(float stable_weight_g, float delta_g)
{
    ESP_LOGI(TAG, "weight stabilized at %.2f g (delta %.2f g), capturing frame", stable_weight_g, delta_g);

    if (capture_frame_to_latest_buffer(s_ai_input_video) != ESP_OK) {
        ESP_LOGW(TAG, "failed to capture frame after weight trigger");
        return ESP_FAIL;
    }

    if (ai_submit_latest_frame(stable_weight_g, delta_g) != ESP_OK) {
        ESP_LOGW(TAG, "AI task is not ready");
        return ESP_FAIL;
    }
...
static void weight_capture_trigger_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "weight trigger task started, waiting for food weight changes");

    hx711_init(EXAMPLE_HX711_DATA_GPIO, EXAMPLE_HX711_SCK_GPIO, HX711_GAIN_128);
    hx711_set_scale(EXAMPLE_HX711_SCALE);
    vTaskDelay(pdMS_TO_TICKS(EXAMPLE_HX711_POWER_UP_DELAY_MS));
    hx711_tare();
    vTaskDelay(pdMS_TO_TICKS(300));

    reference_weight_g = hx711_get_units(EXAMPLE_HX711_AVG_SAMPLES);
    candidate_weight_g = reference_weight_g;
    stable_count = EXAMPLE_WEIGHT_STABLE_COUNT;

    ESP_LOGI(TAG,
             "HX711 ready on DOUT=%d SCK=%d, scale %.2f, reference weight %.2f g, threshold %.2f g, stable window %.2f g x %d",
             EXAMPLE_HX711_DATA_GPIO,
             EXAMPLE_HX711_SCK_GPIO,
             EXAMPLE_HX711_SCALE,
             reference_weight_g,
             EXAMPLE_WEIGHT_CHANGE_THRESHOLD_G,
             EXAMPLE_WEIGHT_STABLE_DELTA_G,
             EXAMPLE_WEIGHT_STABLE_COUNT);

    while (1) {
        float current_weight_g = hx711_get_units(EXAMPLE_HX711_AVG_SAMPLES);
        float candidate_delta_g = absf(current_weight_g - candidate_weight_g);
        float reference_delta_g = absf(current_weight_g - reference_weight_g);

        if (candidate_delta_g <= EXAMPLE_WEIGHT_STABLE_DELTA_G) {
            candidate_weight_g = (candidate_weight_g * stable_count + current_weight_g) / (stable_count + 1);
            if (stable_count < EXAMPLE_WEIGHT_STABLE_COUNT) {
                stable_count++;
            }
        } else {
        ...
        if (!change_detected && reference_delta_g >= EXAMPLE_WEIGHT_CHANGE_THRESHOLD_G) {
            change_detected = true;
            stable_count = 1;
            candidate_weight_g = current_weight_g;
            ESP_LOGI(TAG, "weight change detected: current %.2f g, reference %.2f g", current_weight_g, reference_weight_g);
        }

        if (change_detected && stable_count >= EXAMPLE_WEIGHT_STABLE_COUNT) {
            int64_t now_us = esp_timer_get_time();
            float stable_delta_g = absf(candidate_weight_g - reference_weight_g);

            if (stable_delta_g >= EXAMPLE_WEIGHT_CHANGE_THRESHOLD_G) {
                if ((now_us - last_trigger_time_us) >= (EXAMPLE_TRIGGER_COOLDOWN_MS * 1000LL)) {
                    ...
                    ESP_LOGI(TAG, "weight stabilized but trigger is cooling down");
                }
            } else {
                ESP_LOGI(TAG, "weight returned near reference before stabilizing, skip capture");
            }

            change_detected = false;
        }

        vTaskDelay(pdMS_TO_TICKS(EXAMPLE_WEIGHT_POLL_INTERVAL_MS));
    }
}
...
static esp_err_t init_web_cam_video(web_cam_video_t *video, const web_cam_video_config_t *config, int index)
{
    ...
    if (video->pixel_format == V4L2_PIX_FMT_RGB565) {
        ESP_GOTO_ON_ERROR(latest_frame_init(video), fail0, TAG, "failed to init latest frame buffer");
    }
...
static esp_err_t start_local_camera(const web_cam_video_config_t *config, int config_count)
{
    web_cam_t *web_cam;
...
static esp_err_t start_ai_pipeline(web_cam_t *web_cam)
{
    web_cam_video_t *ai_video;
    BaseType_t task_ok;

    ai_video = get_ai_input_video(web_cam);
    ESP_RETURN_ON_FALSE(ai_video, ESP_ERR_NOT_FOUND, TAG, "no RGB565 camera available for AI input");

    ESP_LOGI(TAG, "AI input video%d: width=%" PRIu32 " height=%" PRIu32 " format=" V4L2_FMT_STR,
             ai_video->index, ai_video->width, ai_video->height, V4L2_FMT_STR_ARG(ai_video->pixel_format));
    s_ai_input_video = ai_video;

    esp_task_wdt_config_t twdt_config = {
        .timeout_ms = AI_TASK_WDT_TIMEOUT_MS,
        .idle_core_mask = (1 << portNUM_PROCESSORS) - 1,
        .trigger_panic = true,
    };
    ESP_RETURN_ON_ERROR(esp_task_wdt_reconfigure(&twdt_config), TAG, "failed to reconfigure task watchdog");

    if (s_ai_task_handle) {
        return ESP_OK;
    }

    task_ok = xTaskCreatePinnedToCore(ai_inference_task, "ai_task", 1024 * 12, NULL, 5, &s_ai_task_handle, 1);
    ESP_RETURN_ON_FALSE(task_ok == pdPASS, ESP_ERR_NO_MEM, TAG, "failed to create AI task");

    if (!s_weight_trigger_enabled) {
        task_ok = xTaskCreate(weight_capture_trigger_task, "weight_trigger", 1024 * 4, NULL, 4, NULL);
        ESP_RETURN_ON_FALSE(task_ok == pdPASS, ESP_ERR_NO_MEM, TAG, "failed to create weight trigger task");
        s_weight_trigger_enabled = true;
    }
...
static void network_web_server_task(void *arg)
{
    (void)arg;

    ESP_LOGI(TAG, "network task started; local camera, weight and AI tasks do not wait for Wi-Fi");

    esp_err_t ret = example_connect();
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "network connect failed: %s, web server is not started", esp_err_to_name(ret));
        vTaskDelete(NULL);
        return;
    }
    ...
    if (!s_web_cam) {
        ESP_LOGW(TAG, "camera is not ready, web server is not started");
        vTaskDelete(NULL);
        return;
    }

    ret = http_server_init(s_web_cam);
    if (ret == ESP_OK) {
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(start_local_camera(config, config_count));
    if (start_ai_pipeline(s_web_cam) != ESP_OK) {
        ESP_LOGW(TAG, "AI pipeline is not started");
    }

    BaseType_t task_ok = xTaskCreate(network_web_server_task, "net_web", 1024 * 5, NULL, 3, NULL);
    if (task_ok != pdPASS) {
        ESP_LOGW(TAG, "failed to create network task; web server is disabled");
    }

    ESP_LOGI(TAG, "Local camera, weight trigger and AI pipeline startup finished");
}
