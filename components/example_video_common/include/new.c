...
extern esp_err_t ai_submit_latest_frame(void);
...
static void camera_capture_trigger_task(void *arg)
{
    ESP_LOGI(TAG, "capture trigger task started, press Enter in monitor to capture one frame for AI");
    ...
    while (1) {
        ...
        if (capture_frame_to_latest_buffer(s_ai_input_video) == ESP_OK) {
            if (ai_submit_latest_frame() != ESP_OK) {
...
static esp_err_t start_ai_pipeline(web_cam_t *web_cam)
{
    ...
    task_ok = xTaskCreate(camera_capture_trigger_task, "cap_trigger", 1024 * 4, NULL, 4, NULL);
    ESP_RETURN_ON_FALSE(task_ok == pdPASS, ESP_ERR_NO_MEM, TAG, "failed to create capture trigger task");
